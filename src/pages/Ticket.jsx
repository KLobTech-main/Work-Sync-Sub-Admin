...
const Ticket = () => {
  const [tickets, setTickets] = useState([
    { id: 1, email: 'user1@example.com', title: 'Login Issue', description: 'Cannot login to the system', status: 'OPEN', priority: 'High' },
    { id: 2, email: 'user2@example.com', title: 'UI Bug', description: 'Button not working on homepage', status: 'IN_PROGRESS', priority: 'Medium' },
              ...
              {filteredTickets.length > 0 ? (
                filteredTickets.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((ticket) => (
                  <TableRow key={ticket.id}>
                    ...
                    <TableCell>
                      <span style={{ fontWeight: 'bold', color: ticket.status === 'OPEN' ? 'red' : ticket.status === 'RESOLVED' ? 'green' : 'orange' }}>
                        {ticket.status}
                      </span>
                    </TableCell>
